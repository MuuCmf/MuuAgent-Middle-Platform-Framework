...
@Injectable()
export class UseSkillTool implements IAgentTool {
  private readonly logger = new Logger(UseSkillTool.name);

  readonly name = 'use_skill';

  readonly definition: ToolDefinition = {
    name: 'use_skill',
    description: '按需加载指定技能的完整指令。当需要技能的详细操作步骤、API参数格式或执行注意事项时调用此工具。',
    parameters: {
      type: 'object',
      properties: {
        skill_name: {
