...
export class LoadReferenceTool extends BaseTool {
  readonly name = 'load_reference';

  readonly definition: ToolDefinition = {
    name: 'load_reference',
    description:
      '加载技能的参考文档（references/ 中的文件）。当技能指令中引用了附加文档时使用。',
    parameters: {
      type: 'object',
      properties: {
        ...
        reference_path: {
          type: 'string',
          description: '参考文档的相对路径，如 api-docs.md',
        },
      },
  ...
  async execute(args: Record<string, unknown>, _context: ToolExecutionContext): Promise<unknown> {
    ...
    const referencePath = this.getArg<string>(args, 'reference_path');
