...
@Injectable()
export class ReactReasoningEngine implements IReasoningEngine {
  readonly mode = ReasoningMode.REACT;
  private readonly logger = new Logger(ReactReasoningEngine.name);
  ...
  async executeStream(context: ExecutionContext, emitter: StreamEmitter): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`[ReAct] 执行完成, 耗时: ${context.totalCostMs}ms`);
    } catch (error) {
      const errorMsg = parseAiError(error);
      const errorCode = getErrorCode(error);
      this.logger.error(`[ReAct] 执行失败: ${errorMsg}`);
